...
describe SwipelyBuilder::Deb do
  describe '#create_package!' do
    subject do
      SwipelyBuilder::Deb.new do
    ...
    context 'when it has a docker', :docker do
      ...
      it 'builds the docker image and adds it to the deb' do
        subject.create_package!
        `dpkg --contents #{filename}`
            .lines.grep(/swipely_builder-deb_test-image\.tgz/).should_not be_empty
      end
    end
    ...
    context 'when it has files' do
      ...
      before do
        ...
        subject.file './lib/swipely_builder/.', '/etc/deploys'
      ...
      it 'adds the file to deb' do
        ...
        expect(contents.lines.grep(/\/etc\/specs\/spec\/swipely_builder_spec.rb/)).to_not be_empty
      end
    end
  ...
  describe '#exists?' do
    ...
    context 'when the object does exist' do
      before do
        SwipelyBuilder::AWS.s3.stub(:head_object).and_return {}
      end
    ...
    context 'when the object does not exist' do
      before do
        SwipelyBuilder::AWS.s3.stub(:head_object).and_raise(Excon::Errors::NotFound.new "NotFound")
      end
  ...
  describe '#upload_to_s3' do
    ...
    context 'when the s3_bucket is nil' do
      it 'does nothing' do
        SwipelyBuilder::AWS.should_not_receive(:s3)
    ...
    context 'when the s3_bucket is present' do
      ...
      context 'when the package has been created' do
        ...
        it 'creates the s3 bucket' do
          subject.upload_to_s3
          SwipelyBuilder::AWS.s3.get_bucket(bucket_name).body.should_not be_nil
        end

        it 'inserts the deb package into that bucket' do
          subject.upload_to_s3
          SwipelyBuilder::AWS.s3.get_bucket(bucket_name, subject.s3_object_name).body.should_not be_nil
        end
      end
    end
  end
